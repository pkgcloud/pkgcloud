...
//var tests = {
//  getVersion: {
//    test: function (provider, client, next) {
...
//      }
//    },
//    description: 'the setWait() method on a flavor should timeout'
//  },
//  getImages: {
//    test: function (provider, client, next) {
//      return function (done) {
...
//
//
//// Run through each enabled provider, and then execute all of the tests in the provider.
//// As we want to ensure each provider is run in series, we have to wrap them each in a
//// discrete loop to ensure we don't double up, which gives Nock difficulties
//
//async.forEachSeries(providers, function(provider, seriesNext) {
//  var client = helpers.createClient(provider, 'compute');
//
//  testContext = {};
//
//  async.forEachSeries(_.keys(tests), function(key, next) {
//    describe('pkgcloud/common/compute/base [' + provider + ']', function() {
//      it(tests[key].description, tests[key].test(provider, client, next));
//    });
//  }, function() {
//    seriesNext();
//  });
