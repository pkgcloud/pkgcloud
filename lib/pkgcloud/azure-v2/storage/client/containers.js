...
function createContainer(options, callback) {
  ...
  async.waterfall([
    ...
    (creds, next) => {
      azureApi.setup(client, next);
    },
    (next) => {
      var parameters = {
        location: options.location || client.azure.location,
        sku: {
          name: options.accountType || constants.DEFAULT_STORAGE_SKU,
        },
        kind: 'storage'
      };
      
      var storageClient = new StorageManagementClient(client.azure.credentials, client.config.subscriptionId);
      return storageClient.storageAccounts.create(client.config.resourceGroup, containerName, parameters, (err, result) => {
        return err
          ? next(err)
          : next(null, new (storage.Container)(client, result));
      });
