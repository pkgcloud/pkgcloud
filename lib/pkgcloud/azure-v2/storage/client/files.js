 ...
 * @param callback
 */
exports.TODO_removeFile = function (container, file, callback) {
...
exports.getFiles = function (container, options, callback) {
  ...
  async.waterfall([
    (next) => {
      storageApi.getBlobService(client, azureContainer, containerName, next);
    },
    (blobService, next) => {

      var aggregateBlobs = (err, result, cb) => {
        if (err) {
          cb(err);
        } else {
      ...
      blobService.listBlobsSegmented(azureContainer, null, function(err, result) {
        aggregateBlobs(err, result, (err, blobs) => {
          return err ?
            next(err) :
            next(null, blobs.map(blob => new client.models.File(client, blob)));
        });
      });
...
exports.getFile = function (container, file, options, callback) {
  ...
  async.waterfall([
    (next) => {
      storageApi.getBlobService(client, azureContainerName, containerName, next);
    },
    (blobService, next) => {

      blobService.getBlobProperties(azureContainerName, fileName, (err, properties, status) => {
        return err ?
          next(err) : (!status || !status.isSuccessful) ?
          next(new Error(`status is not successfull: ${JSON.stringify(status || null)}`)) :
          next(null, new client.models.File(client, properties));
      });
    }
  ], callback);
};
...
exports.download = function (options, callback) {
  ...
  async.waterfall([
    ...
    (_blobService, next) => {
      ...
      blobService.getBlobProperties(azureContainerName, blobName, (err, properties, status) => {
        return err ?
          next(err) : (!status || !status.isSuccessful) ?
          next(new Error(`status is not successfull: ${JSON.stringify(status || null)}`)) :
