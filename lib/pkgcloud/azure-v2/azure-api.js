...
function login(setupLocation, callback) {
  ...
  msRestAzure.loginWithServicePrincipalSecret(
    ...
    (err, credentials) => {
      ...
      if (setupLocation) {
        return self.setup(credentials, err => callback(err, credentials));
      } else {
...
function setupLocation(credentials, callback) {
  ...
  if (self.config.resourceGroup) {
    ...
    resourceClient.resourceGroups.get(self.config.resourceGroup, (err, result) => {
...
function bind(client) {
  ...
  client.deploy = templates.deploy.bind(this);
}
