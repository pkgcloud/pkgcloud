  ...
  async.waterfall([
    ...
    (credentials, next) => {
      ...
      client.virtualMachines.list(self.config.resourceGroup, (err, results) => {
        return err
          ? next(err)
          : next(null, results.map(res => new self.models.Server(self, res)));
      });
    }
  ], callback);
};
  ...
  async.waterfall([
    ...
    (credentials, next) => {
      ...
      client.virtualMachines.get(self.config.resourceGroup, serverId, options, (err, result) => {
        return err
          ? next(err)
          : next(null, new self.models.Server(self, result));
      });
    }
  ], callback);
};
...
function createServer(options, callback) {
  ...
  templates.deploy(self, templateName, options, (err, result) => {
    return err ?
      callback(err) :
