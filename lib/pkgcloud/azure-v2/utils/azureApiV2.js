...
exports.createServer = function (client, options, callback) {
  ...
  async.waterfall([
    ...
    function (next) {
      loginToAzure(client, options, next);
    },
    function (next) {
 ...
 */
var loginToAzure = function (client, options, callback) {
  ...
  msRestAzure.loginWithServicePrincipalSecret(config.spClientId, config.spSecret, config.spDomain, function (err, credentials) {
      ...
      if (err) {
        errs.handle(
          errs.create({ message: 'There was a problem connecting to azure: ' + err }),
          callback
        );
      }
...
var createVM = function (client, options, vmOptions, callback) {
  ...
  switch (options.image.OS.toLowerCase()) {
    ...
    case 'windows':
      ...
      break;
    default:
      callback(errs.create({message: 'Unknown Image OS: ' + options.image.OS}));
      break;
  }
};
