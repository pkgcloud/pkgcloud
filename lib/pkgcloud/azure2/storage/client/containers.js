...
exports.createContainer = function (container, callback) {
  ...
  var onError = function(err) {
    // if container already exists, just get the container.
    if(err.statusCode === 409) {
      self.getContainer(container, callback);
    } else {
      callback(err);
    }
  };

  this.xmlRequest('PUT', [containerName,'?restype=container'], onError, function (body, res) {
