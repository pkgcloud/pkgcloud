...
var createServer = exports.createServer = function(client, options, callback) {
  ...
  async.waterfall([
    ...
    function(next) {
      ...
      getOSImage(options.image, function(err, res) {
        if(err) {
          next(err);
        } else {
          image = res;
...
var validateCreateOptions = function(options, config, callback) {
  ...
  ['username', 'password', 'name','location','storageAccount'].forEach(function (member) {
...
var makeTemplateRequest = function(client, path, templateName, params, callback) {
  ...
  async.waterfall([
    function(next) {
      // load the createHostedService eml template
      templates.load('createHostedService.xml', next);
    },
    function(template, next){
      ...
      client.request({
        method: 'POST',
        path: path,
        body:_.template(template, params), // compile template with params
        headers: headers
