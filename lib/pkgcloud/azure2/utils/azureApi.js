...
var captureServer = function(client, serverName, targetImageName, next) {
  ...
  var path = client.subscriptionId + '/services/hostedservices/' + serverName +
    ...
    '/roleinstances/' + serverName + '/Operations';

  // set up deployment config templates params
  var label = new Buffer(serverName).toString('base64'); //TODO: which name?

  var configParams = {
    NAME: options.name, //TODO: which name?
    LABEL_BASE64: label,
    USERNAME: client.config.azure.username,
    PASSWORD: client.config.azure.password,
    SSH_CERTIFICATE_FINGERPRINT: vmOptions.sshCertInfo.fingerprint
  ...
  makeTemplateRequest(client, path, 'captureLinuxRole.xml', configParams, callback);
};
