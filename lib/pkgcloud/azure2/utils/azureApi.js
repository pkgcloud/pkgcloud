...
var createLinuxVM = function(client, options, vmOptions, callback) {
  ...
  var configSets = new Buffer(options.name).toString('base64');
  var createConfigParams = {
    NAME: options.name,
    LABEL_BASE64: label,
    CONFIGURATION_BASE64: configSets
  };

  // async execute the following tasks one by one and bail if there is an error
  async.waterfall([
    function(next) {
      templates.compile("linuxConfigSet.xml", linuxConfigParams, function(err, result) {
        if(err) {
          next(err);
        } else {
          deploymentConfigParams.OS_CONFIG_SET = result;
          next(null);
        }
      });
    },
    function(next) {
      templates.compile("sshNetworkConfigSet.xml", sshNetworkConfigParams, function(err, result) {
        if(err) {
          next(err);
        } else {
          deploymentConfigParams.NETWORK_CONFIG_SET = result;
          //makeTemplateRequest(client, path, 'deploymentConfig.xml', deploymentConfigParams, next);
          next(null);
        }
      });
    },
    function(next) {
      templates.compile("deploymentConfig.xml", deploymentConfigParams, function(err, result) {
        if(err) {
          next(err);
        } else {
          deploymentConfigParams.NETWORK_CONFIG_SET = result;
