...
var azureApi = require('../azure/utils/azureApi'),
  ...
  SASToken = require('../azure/utils/sasToken'),
...
exports.storageSignature = function storageSignature(req, options) {
  ...
  if ((typeof options.storageAccessKey !== 'string') && (typeof options.sasToken !== 'string')) {
    throw new TypeError('`storageAccessKey` is a required argument for azure-signature');
  }

  var sharedKey;
  if (typeof options.storageAccessKey == 'string') {
    sharedKey = new SharedKey(options.storageAccount, options.storageAccessKey);
  }
  else {
    sharedKey = new SASToken(options.storageAccount, options.sasToken);
  }
