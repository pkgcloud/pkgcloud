  ...
  var couch = {
      ...
      request(options, function (err, response, body) {
        ...
        if (response.statusCode === 201) {
          if (result.ok === true) {
            // We got OK from the server, lets starting polling to see if the couch is ready
...
Client.prototype._checkCouch = function (couchName, callback) {
  ...
  // Poll the couch with a GET every interval to determine if couch is up yet
  // We perform a poll since there is no notification event from couchone
