...
Identity.prototype.authorize = function (options, callback) {
  ...
  request(authenticationOptions, function (err, response, body) {
    if (err || response.statusCode !== 200) {
      return callback(err || new Error('Invalid response code: ' + response.statusCode));
    }
...
Identity.prototype.validateToken = function(token, belongsTo, callback) {
  ...
  function validate() {
    var options = {
      uri: urlJoin(self.serviceCatalog.getServiceByType('identity').getEndpointUrl({ admin: true }), '/tokens', token),
      method: 'GET',
      headers: {
