...
Identity.prototype.authorize = function (options, callback) {
  ...
  self.emit('log::debug', 'Sending client authorization request', authenticationOptions);
  ...
  request(authenticationOptions, function (err, response, body) {
    ...
    self.emit('log::debug', 'Provider Authentication Response', {
...
Identity.prototype._buildAuthenticationPayload = function () {
  ...
  self.emit('log::debug', 'Building Openstack Identity Auth Payload');
...
Identity.prototype._parseIdentityResponse = function (data) {
  ...
  if (self.options.username !== data.access.user.name) {
    throw new Error('Username did not match provided username!');
  }
