...
exports.getUser = function getUser(instance, user, callback) {
  var self = this,
      instanceId,
      userId;
  ...
  instanceId = instance instanceof Instance ? instance.id : instance;
  userId = user instanceof User ? user.name : user;
...
// #### options['password'] {string} The password for the user to create (required)
// #### options['instance'] {string | Object} The instance could be the ID for the instance or a instance of Instance class (required)
exports.changePassword = function changePassword(options, callback) {
  var instanceId,
      users = [],
      regex = /^\s|^\?|^@|^#| \w* \d* |'|"|`|;|,|\\|\/| \s$/;
  ...
  if (!options || typeof options === 'function') {
    return errs.handle(errs.create({
      message: 'Options required for change user password.'
    }), options);
  }

  instanceId = options.instance instanceof Instance ? options.instance.id : options.instance;

  // Check for invalid characters in username and password
  if (regex.test(options.username)) {
    return errs.handle(errs.create({
      message: 'Invalid characters in username ' + options.username
    }), callback);
  }
  if (regex.test(options.password)) {
    return errs.handle(errs.create({
      message: 'Invalid characters in password ' + options.password
    }), callback);
  }
  // Check username character limit
...
// ### @options {Object|Array} Set of options can be one user object or an
// ### array of user objects.
// #### options['user'] {string | Object} Could be the name of the user or a instance of User class (required)
// #### options['databases'] {string | array} Name or instances of databases that the user can access (required)
// #### options['instance'] {string | Object} The instance could be the ID for the instance or a instance of Instance class (required)
exports.grantAccess = function grantAccess(options, callback) {
  var instanceId,
      userId,
      dbs = [];
  ...
  if (!options || typeof options === 'function') {
    return errs.handle(errs.create({
      message: 'Options required for grant access.'
    }), options);
  }

  instanceId = options.instance instanceof Instance ? options.instance.id : options.instance;
  userId = options.user instanceof User ? options.user.name : options.user;

  // If 'databases' is an array so we push each name in databases list
  if (options && options['databases'] &&
      options['databases'] instanceof Array &&
      options['databases'].length > 0) {
    options['databases'].forEach(function (item) {
      if (typeof item === 'string') {
        dbs.push({'name' : item});
      } else if (item instanceof Database) {
        dbs.push({'name' : item.name});
      }
    });
  }

  if (options && options['database'] && typeof options['database'] === 'string') {
    dbs.push({ 'name' : options['database']});
  }

  if (options && options['database'] && options['database'] instanceof Database) {
    dbs.push({ 'name' : options['database'].name});
  }

  if (options && options['databases'] && typeof options['databases'] === 'string') {
    dbs.push({ 'name' : options['databases']});
  }

  if (options && options['databases'] && options['databases'] instanceof Database) {
    dbs.push({ 'name' : options['databases'].name});
  }

  this._request({
    method: 'PUT',
    path: 'instances/' + instanceId + '/users/' + userId + '/databases',
    body: {
      databases: dbs
    }
  }, function (err, body, response) {
...
// Remove a user permission to use a given database.
// ### @options {Object|Array} Set of options can be one user object or an
// ### array of user objects.
// #### options['user'] {string | Object} Could be the name of the user or a instance of User class (required)
// #### options['database'] {string | Object} Name or instance of database that the user CANNOT access (required)
// #### options['instance'] {string | Object} The instance could be the ID for the instance or a instance of Instance class (required)
exports.revokeAccess = function revokeAccess(options, callback) {
  var instanceId,
      userId,
      database;
  ...
  if (!options || typeof options === 'function') {
    return errs.handle(errs.create({
      message: 'Options required for revoke access.'
    }), options);
  }
  ...
  this._request({
    method: 'DELETE',
    path: 'instances/' + instanceId + '/users/' + userId + '/databases/' + database,
  }, function (err, body, response) {
    return err
      ? callback(err)
      : callback(null, response);
