...
exports.createServer = function createServer(details, callback) {
  ...
  if (options.imageRef) {
    createOptions.body.server.imageRef = options.imageRef;
  }

  if (options.flavorRef) {
    createOptions.body.server.flavorRef = options.flavorRef;
  }
  ...
  if (options.metadata) {
    createOptions.body.server.metadata = options.metadata;
  }

  if (options.min_count) {
    createOptions.body.server.min_count = options.min_count;
  }

  if (options.max_count) {
    createOptions.body.server.max_count = options.max_count;
  }
