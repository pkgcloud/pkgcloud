 ...
 *
 */
var base = require('../../../core/compute'), pkgcloud = require('../../../../../lib/pkgcloud'), errs = require('errs'), urlJoin = require('url-join'), util = require('util'), _ = require('underscore'), Server = require('../server').Server, compute = pkgcloud.providers.openstack.compute;
 ...
 * @param {String}
 *          formatString String formatter for the error message
 * @param {Function}
 *          callback
 * @returns {boolean}
 */
  ...
  return !required.some(function(item) {
    if (typeof (options[item]) === 'undefined') {
      errs.handle(errs.create({
        message : util.format(formatString, item)
      }), callback);
      return true;
    }
    return false;
 ...
 * 
 * @description exported helper function that is wrapped by a number of other
 *              more specific functions. Calls the standard server action api
 *              for any manner of tasks
 * 
 * @param {object|String}
 *          server The server or serverId to get volumes for
 * @param {object}
 *          body The body of the server action to perform
 * @param {function}
 *          callback
 * @returns {*}
 */
exports._doServerAction = function(server, body, callback) {
  var self = this, serverId = server instanceof Server ? server.id : server;
 ...
 *          [options] A set of options for the getServers call
 * @param {function}
 *          callback f(err, servers) where servers is an array of Server
 * @returns {*}
 */
exports.getServers = function getServers(options, callback) {
  ...
  requestOptions.qs = _.pick(options, 'image', 'flavor', 'name', 'status',
      'marker', 'limit', 'changes-since');
 ...
 * client.createServer
 * 
 * @description Creates a server with the specified options. The flavor
 *              properties of the options can be instances of Flavor OR ids to
 *              those entities in OpenStack.
 * 
 * @param {object}
 *          details
 * @see http://developer.openstack.org/api-ref-compute-v2.1.html#os-compute-v2.1
 * @param callback
 * @returns {request|*}
 */
exports.createServer = function createServer(details, callback) {
  ...
  details = details || {};
  if (!validateProperties([ 'imageRef', 'flavorRef', 'name' ], details,
      'options.%s is a required argument.', callback)) {
    return;
  }
  var self = this, createOptions = {
    method : 'POST',
    path : _urlPrefix,
    body : {
      server : details
 ...
 * 
 * @param {String|object}
 *          server The server or serverId to fetch
 * @param {Function}
 *          callback
 * @returns {request|*}
 */
exports.getServer = function getServer(server, callback) {
  var self = this, serverId = server instanceof base.Server ? server.id
      : server;
 ...
 * 
 * @param {String|object}
 *          server The server or serverId to delete
 * @param {Function}
 *          callback
 * @returns {request|*}
 */
exports.destroyServer = function destroyServer(server, callback) {
  ...
  return this._request({
    ...
    return err ? callback(err) : callback(err, {
      ok : serverId
    });
  });
};
 ...
 * 
 * @param {String|object}
 *          server The server or serverId to reboot
 * @param {object}
 *          [options] Optionally determine if it's a hard or soft reboot
 * @param {String}
 *          [options.type] HARD or SOFT reboot. Default is SOFT
 * @param {Function}
 *          callback
 * @returns {*}
 */
exports.rebootServer = function rebootServer(server, options, callback) {
  ...
  return this._doServerAction.call(this, server, {
    reboot : options
  }, callback);
};

/**
 * client.rebuildServer FIXME: it has to be tested with OpenStack API v2.1
 * 
 * @description Rebuild the provider server
 * 
 * @param {String|object}
 *          server The server or serverId to rebuild
 * @param {String|object}
 *          options The image or imageId to use in the rebuild (for backwards
 *          compatibility)
 * @param {String|object}
 *          options.image The image or imageId to use in the rebuild
 * @param {String}
 *          options.accessIPv4 The IP version 4 address of the server.
 * @param {String}
 *          options.accessIPv6 The IP version 6 address of the server.
 * @param {String}
 *          options.adminPass The administrator password for the server.
 * @param {object}
 *          options.metadata Metadata key/value pairs.
 * @param {array}
 *          options.personality Personality files - path and contents
 * @param {String}
 *          options.personality[n].path Personality file path
 * @param {String}
 *          options.personality[n].contents Personality file contents
 * @param {String}
 *          options['OS-DCF:diskConfig'] The disk configuration value ("AUTO" |
 *          "MANUAL").
 * @param {Function}
 *          callback
 * @returns {*}
...
exports.rebuildServer = function rebootServer(server, options, callback) {
  ...
  if (options instanceof base.Image) {
    ...
    rebuildBody = _.pick(options, [ 'accessIPv4', 'accessIPv6', 'adminPass',
        'metadata', 'personality' ]);
    rebuildBody.imageRef = options.image instanceof base.Image ? options.image.id
        : options.image;
  } else {
...
/**
 * client.resizeServer FIXME: it has to be tested with OpenStack API v2.1
 * 
 * @description Resize the provider server
 * 
 * @param {String|object}
 *          server The server or serverId to resize
 * @param {String|object}
 *          flavor The flavor or flavorId to use in the resize
 * @param {Function}
 *          callback
 * @returns {*}
 */
...
/**
 * client.confirmServerResize FIXME: it has to be tested with OpenStack API v2.1
 * 
 * @description Confirm the resize operation for the provided server
 * 
 * @param {String|object}
 *          server The server or serverId
 * @param {Function}
 *          callback
 * @returns {*}
 */
...
/**
 * client.revertServerResize FIXME: it has to be tested with OpenStack API v2.1
 * 
 ...
 *          server The server or serverId
 * @param {String}
 *          [type] Optionally provide the type of addresses by network label
 * @param {Function}
 *          callback
 * @returns {*}
 */
exports.getServerAddresses = function(server, type, callback) {
  ...
  this._request(options, function(err, body) {
    return err ? callback(err) : callback(null, body.addresses || body);
  });
};
