...
Server.prototype._setProperties = function (details) {
  ...
  this.keyName   = details.key_name;
  this.securityGroups = details.security_groups || [];
    ...
    this.addresses = Object.keys(this.addresses)
      ...
      .reduce(function (all, interfaces) {
        Object.keys(interfaces).map(function (interface) {
          return {
            addr: interfaces[interface].addr,
            type: interfaces[interface]['OS-EXT-IPS:type']
          };
        })
        .forEach(function (interface) {
          return compute.isPrivate(interface.addr) && interface.type === 'fixed'
            ? all['private'].push(interface.addr)
            : all['public'].push(interface.addr);
        });
...
Server.prototype.toJSON = function() {
  return _.pick(this, ['id', 'name', 'status', 'hostId', 'adminPass', 'addresses',
    ...
    'created', 'updated', 'keyName', 'securityGroups']);
};
