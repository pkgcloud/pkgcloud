...
exports.createService = function (details, callback) {
  ...
  return self._request(createOptions, function (err, body) {
    ...
    if (!body) {
      return new Error('Service not passed back from Openstack.');
    }

    // since createService returns an href to the service, lets go fetch it
    self.getService(body.name, callback);
  });
};
