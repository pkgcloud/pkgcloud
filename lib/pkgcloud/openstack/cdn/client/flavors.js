 ...
 *
 * @param {String|object}   flavor    The flavor or flavorName to fetch
 * @param {Function}        callback
 * @returns {request|*}
 */
exports.getFlavor = function (flavor, callback) {
  var self = this,
    path = flavor instanceof cdn.Flavor
      ? urlJoin(_urlPrefix, flavor.name)
...
exports.createFlavor = function (details, callback) {
  ...
  return self._request(createOptions, function (err, body) {
    ...
    if (!body) {
      return new Error('Flavor not passed back from Openstack.');
    }

    // since createFlavor returns an href to the flavor, lets go fetch it
    self.getFlavor(body.id, callback);
  });
};
