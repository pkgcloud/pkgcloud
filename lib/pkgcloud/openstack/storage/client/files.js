 ...
 * client.copy
 *
 * @description copies a file to another location in the container.
 *
 * @param {String|object}     container     the container or containerName
 * @param {String|object}     file          the file or fileName to copy
 * @param {String|object}     destination   the destination or destinationName to copy to
 * @param callback
 */
exports.copy = function (container, file, destination, callback) {
	var self = this,
		containerName = container instanceof self.models.Container ? container.name : container;

	var copyOptions = {
		method: 'COPY',
		uri: file instanceof self.models.File ? file.fullPath : this._getUrl({
			container: containerName,
			path: file
		}),
		headers: {
			'DESTINATION': destination instanceof self.models.File ?
				urlJoin('/', destination.container, destination.name) :
				urlJoin('/', containerName, destination)
		}
	};
