...
exports.createContainer = function (options, callback) {
  ...
  if (options.metadata) {
     _.merge(headers, self.serializeMetadata(self.CONTAINER_META_PREFIX, options.metadata));
  }

  createContainerOpts.headers = headers;
...
/**
 * client.updateContainerHeaders
 *
 * @description Updates the headers in the specified `container` in
 * the storage container associated with this instance.
 *
 * @param {Container}     container     the container instance
 * @param callback
 */
exports.updateContainerHeaders = function(container, callback) {
  this._updateContainerHeaders(container, container.headers, callback);
};

/**
 * client.removeContainerHeaders
 *
 * @description Removes the provided `headers` in the specified
 * `container` in the storage container associated with this instance.
 *
 * @param {Container}     container     the container instance
 * @param {object}        headersToRemove     the headers to remove from the container
 * @param callback
 */
exports.removeContainerHeaders = function(container, headersToRemove, callback) {
  var headers = {};
  var prefix = this.CONTAINER_HEADER_PREFIX.toLowerCase();
  for (var key in headersToRemove) {
    if (headersToRemove.hasOwnProperty(key) &&
        0 === key.toLowerCase().indexOf(prefix)) {
      var targetKey = this.CONTAINER_REMOVE_HEADER_PREFIX + key.slice(prefix.length);
      headers[targetKey] = headersToRemove[key];
    }
  }
  this._updateContainerHeaders(container, headers, callback);
};
...
exports.updateContainerMetadata = function (container, callback) {
  this._updateContainerHeaders(container,
    this.serializeMetadata(this.CONTAINER_META_PREFIX, container.metadata),
    callback);
};

/**
 * client.removeContainerMetadata
 *
...
/**
 * client._updateContainerHeaders
 *
 * @description Convenience function for updating container headers
 */
exports._updateContainerHeaders = function(container, headers, callback) {
  var self = this;
  ...
  this._request(updateContainerOpts, function (err, body, res) {

    if (!err) {
      var newHeaders = _.extend({}, container.headers, headers, res.headers);
      container._setProperties({
          headers: newHeaders
      });
    }
...
exports._updateContainerMetadata = exports._updateContainerHeaders;
