...
exports.getContainer = function (container, callback) {
  ...
  this._request({
    ...
    if ('x-container-read' in res.headers) {
      details.readacl = res.headers['x-container-read'];
    }

    if ('x-container-write' in res.headers) {
      details.writeacl = res.headers['x-container-write'];
    }
 ...
 * @param {String}    options.readacl   sets read acl
 * @param {String}    options.writeacl  sets write acl
 * @param callback
 */
exports.createContainer = function (options, callback) {
  ...
  if (options.readacl) {
    createContainerOpts.headers['x-container-read'] = options.readacl;
  }

  if (options.writeacl) {
    createContainerOpts.headers['x-container-write'] = options.writeacl;
  }
 ...
 *
 * @description update the settings for a container
 *
 * @param {String|object}     container     the container or containerName
 * @param {object}            options
 * @param {String}            options.readacl   sets read acl
 * @param {String}            options.writeacl  sets write acl
 * @param {String}            options.syncto    sets the destination for container synchronization
 * @param {String}            options.synckey   sets the secret key for container synchronization
 * @param callback
 */
exports.updateContainer = function (container, options, callback) {
  var containerName = container instanceof this.models.Container ? container.name : container,
    self = this;
  ...
  if (options.readacl) {
    updateContainerOpts.headers['x-container-read'] = options.readacl;
  }

  if (options.writeacl) {
    updateContainerOpts.headers['x-container-write'] = options.writeacl;
  }

  if (options.syncto) {
    updateContainerOpts.headers['x-container-sync-to'] = options.syncto;
  }

  if (options.synckey) {
    updateContainerOpts.headers['x-container-sync-key'] = options.synckey;
  }

  self._request(updateContainerOpts, function (err) {
    if (err) {
      return callback(err);
    }
