...
exports.getNetworks  = function (options, callback) {
  ...
  this._request(getNetworkOpts, function (err, body) {
    ...
    return callback(null, body.map(function (network) {
...
exports.getNetwork = function (network, callback) {
  var networkName = network instanceof this.models.Network ? network.name : network,
    self = this;
  ...
  this._request({
    ...
    var details = _.extend({}, body, {
      name: networkName,
      count: parseInt(res.headers['x-network-object-count'], 10),
      bytes: parseInt(res.headers['x-network-bytes-used'], 10)
    });

    details.metadata = self.deserializeMetadata(self.CONTAINER_META_PREFIX, res.headers);

    callback(null, new self.models.Network(self, details));
  });
};
