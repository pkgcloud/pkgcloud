...
exports.getRateSummary = function (options, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }

  var getRateSummaryOpts = {
    path: urlJoin(_urlPrefix, 'report/summary'),
    qs: _.pick(options, ['begin', 'end', 'tenant_id', 'res_type'])
  };

  this._request(getRateSummaryOpts, function (err, body) {
    return err
      ? callback(err)
      : callback(null, body);
  });
