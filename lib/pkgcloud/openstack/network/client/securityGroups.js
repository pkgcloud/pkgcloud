...
/*
  ...
  Add security group update for openstack networking
  (C) 2015 Hopebay tech
  P. Hsuan
 ...
 * @param callback
 */
exports.updateSecurityGroup = function (securityGroup, callback) {
  var self = this,
  sgid = securityGroup.id;
  var sg = _convertSecurityGroupToWireFormat(securityGroup);
  var updateSecurityGroupOpts = {
    method: 'PUT',
    path: urlJoin(securityGroupsResourcePath, sgid),
    contentType: 'application/json',
    body: { 'security_group' : sg}
  };

  self.emit('log::trace', 'Updating Security group', sg);
  this._request(updateSecurityGroupOpts, function (err,body) {
    return err
      ? callback(err)
      : callback(err, new self.models.SecurityGroup(self, body.security_group));
  });
