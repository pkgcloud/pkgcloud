/*
 * loadbalancers.js: Rackspace DNS client loadBalancers functionality
 *
 * (C) 2013 Rackspace
 *      Ken Perkins
 * MIT LICENSE
...
module.exports = {
  ...
  getLoadBalancers: function (callback) {
    ...
    self.request(requestOptions, function (err, body, res) {
      if (err) {
        return callback(err);
      }
  ...
  getLoadBalancer: function(loadBalancer, callback) {
    ...
    self.request({
      ...
      if (err) {
        ...
        return callback(null, new lb.LoadBalancer(self, body.loadBalancer), res);
      }
    });
  },
  ...
  createLoadBalancer: function(details, callback) {
    ...
    self.request(createOptions, function(err, body, res) {
      return err
        ? callback(err)
        : callback(err, new lb.LoadBalancer(body.loadBalancer));
    });
  },
  ...
  updateLoadBalancer: function (loadBalancer, callback) {
    ...
    if (typeof loadBalancer.httpsRedirect === 'boolean') {
      updateOptions.body.loadBalancer.httpsRedirect = loadBalancer.httpsRedirect;
    }

    if (typeof loadBalancer.halfClosed === 'boolean') {
      updateOptions.body.loadBalancer.halfClosed = loadBalancer.halfClosed;
    }
  ...
  getBillableLoadBalancers: function (startTime, endTime, options, callback) {
    ...
    if (options.offest) {
      requestOpts.qs.offset = options.offset;
    }

    if (options.limit) {
      requestOpts.qs.limit = options.limit;
    }

    self.request(requestOpts, function(err, body, res) {
