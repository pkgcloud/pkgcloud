...
exports.createServer = function createServer(options, callback) {
  ...
  ['flavor', 'image', 'name'].forEach(function (member) {
    if (options.required) {
      if(!options[member]) {
        var err = 
          new Error('options.' + required + ' is a required argument.');
         if(callback) return callback(err); throw err;
      }
    }
  });

  // faltam ifs aki caso nao exista
  ...
  return this.request(createOptions, callback, function (body, response) {
    var server = new compute.Server(self, body.server);
