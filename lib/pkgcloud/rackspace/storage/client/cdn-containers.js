...
exports.setTempUrlMetadataKey = function(key, callback) {
  this._request({
    method: 'POST',
    headers: {
...
exports.generateTempUrl = function(container, file, method, time, key, callback) {
  var self = this, split = '/v1';

  time = parseInt(time);

  // We need to make sure the file exists, in part to make sure the file is real, but
  // also to populate the service catalog so we can correctly resolve the file path

  this.getFile(container, file, function(err, object) {
    if (err) {
      callback(err);
      return;
    }
    else if (!object) {
      callback(new Error('Requested object not found'));
    }
