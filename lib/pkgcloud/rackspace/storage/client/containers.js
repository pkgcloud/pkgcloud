...
exports.getContainer = function (container, callback) {
  ...
  this.request({
    ...
    container.metadata = {};
    _.keys(res.headers).forEach(function(key) {
      if (key.indexOf(CONTAINER_META_PREFIX) !== -1) {
        container.metadata[key.split(CONTAINER_META_PREFIX)[1]] = res.headers[key];
      }
    });
...
exports.createContainer = function (options, callback) {
  ...
  if (options.metadata) {
    ...
    _.keys(options.metadata).forEach(function(key) {
      createContainerOpts.headers[CONTAINER_META_PREFIX + key] = options.metadata[key];
    });
  }
...
// #### @options {Container} Container to update in Rackspace Cloudfiles.
// #### @callback {function} Continuation to respond to when complete.
...
exports.updateContainerMetadata = function (container, callback) {
  ...
  _.keys(container.metadata).forEach(function (key) {
    updateContainerOpts.headers[CONTAINER_META_PREFIX + key] = container.metadata[key];
  });
