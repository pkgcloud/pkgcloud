...
exports.getFiles = function (container, options, callback) {
  ...
  self.s3.listObjects(s3Options, function(err, data) {
    ...
    list.forEach(item => {
      ...
      // check Size property to see if this is a file or a folder
      if (item.Size) {
        item.Key = keyParts[keyParts.length - 1]; // for folders like /dir1/file1.ext take only file1.ext
      } else if (options.folder) {
        // if folder is specified for results like /dir1/dir2/ take only `dir2`
        item.Key = `${keyParts[keyParts.length - 2]}/`;
      } else {
        // if no folder is specified (container root) then for get only `dir1` from paths like /dir1/dir2/
        item.Key = `${keyParts[0]}/`;
      }
    });
