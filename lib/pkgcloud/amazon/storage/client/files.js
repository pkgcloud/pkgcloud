...
exports.copy = function (options, callback) {
  var self = this,
      containerName = options.sourceContainer instanceof base.Container ? options.sourceContainer.name : options.sourceContainer,
      destContainerName = options.destinationContainer instanceof base.File ? options.destinationContainer.name : options.destinationContainer,
      destinationFile = options.destinationFile || options.sourceFile;

  self.s3.copyObject({
    Bucket: destContainerName,
    Key: destinationFile instanceof base.File ? destinationFile.name : destinationFile,
    CopySource: options.sourceFile instanceof base.File ? options.sourceFile.fullPath : containerName + '/' + options.sourceFile
  }, function (err, data) {
    return err
        ? callback(err)
        : callback(null, data);
