...
Client.prototype.request = function () {
  ...
  if (!options.path) { 
    return err('No path was provided', errback);
  }
  ...
  function handleRequest(err, res, body) {
    ...
    if (Object.keys(self.failCodes).indexOf(statusCode) !== -1) {
      ...
      err2.provider = self.provider;
      err2.failCode = self.failCodes[statusCode];
  ...
  function sendRequest () {
    ...
    if (self.before) {
      ...
      for(var i in self.before) {
        ...
        } catch (exc) { err(exc,errback); errors = true; break; }
      }
      if (errors) { return; }
    }
    ...
    if (!errback || !callback) {
      ...
      catch (exc1) { return err(exc1); }
    } else {
      try {
        return request(options, handleRequest);
      } catch (exc2) { return err(exc2,errback); }
    }
  }
