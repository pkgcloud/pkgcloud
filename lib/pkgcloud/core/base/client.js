...
Client.request = function () {
  ...
  function sendRequest () {
    ...
    //
    if (this.before) {
      this.before.forEach(function (fn) {
        options = fn.call(self, options);
      });
    }
    ...
    //
    options.uri = self.url(options.path);
    ...
    return request(options, function (err, res, body) {
      ...
      if (Object.keys(self.failCodes).indexOf(statusCode) !== -1) {
        ...
        //
        err = new Error(this.provider + ' Error (' + statusCode + '): ' + failCodes[statusCode]);
