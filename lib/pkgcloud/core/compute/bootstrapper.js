...
// #### @options {Object} Options for the SSH run.
// ####   @options.server     {Server} Server instance to run commands on.
// ####   @options.keys       {Object} SSH keys to use when authenticating.
// ####   @options.commands   {Array}  Set of remote commands to run over SSH.
// ####   @options.address    {string} **Optional** Address to ssh into.
// ####   @options.remoteUser {string} **Optional** Name of the account to use when authenticating.
// ####   @options.tunnel     {Object} **Optional** Enables SSH tunneling through the host. 
...
Bootstrapper.prototype.ssh = function (options) {
  ...
  conn.connect({
    host: address,
    port: 22,
    username: options.remoteUser || this.remoteUser,
    privateKey: options.keys['private'].path
  });
