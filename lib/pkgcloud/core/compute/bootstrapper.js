  ...
  this.readKeys(function (err, keys) {
    ...
    creator.on('active', function (server) {
      ...
      if (options.tunnel) {
        spawn('ssh', [
          '-fNL',
          [options.tunnel.port, pkgcloud.compute.serverIp(server), 22].join(':'),
          [self.remoteUser, options.tunnel.host].join('@')
        ]);
        
        //
        // Remark: Time taken to start the SSH tunnel is not
        //         entirely certain, but this works for now.
        //
...
Bootstrapper.prototype.downloadFile = function (options) {
  options.source = this.remoteUser + '@' +
...
Bootstrapper.prototype.uploadFile = function (options) {
  options.target = this.remoteUser + '@' + 
    (options.address || pkgcloud.compute.serverIp(options.server)) + ':' +
    options.target;
...
Bootstrapper.prototype.ssh = function (options) {
  ...
  if (options.tunnel) {
    options.address = address;
    sshOptions = sshOptions.concat(this._tunnel(options));
  }
...
// * options.tunnel.port: Tunnel port for the specified `options.address`.
//
Bootstrapper.prototype._tunnel = function (options) {  
  return [
    '-o',
...
Bootstrapper.prototype._scpOptions = function (options) {
  ...
  if (options.tunnel) {
    scpOptions = scpOptions.concat(this._tunnel(options));
  }
...
Bootstrapper.prototype._exec = function (command, options) {
  var self     = this,
      server   = options.server,
      address  = pkgcloud.compute.serverIp(options.server),
      emitter  = new events.EventEmitter(),
  ...
  child.stderr.on('data', function (chunk) {
    ...
    chunk.split('\n').filter(Boolean).forEach(function (line) {
      self.emit(['run', command, 'stderr'], { data: line });
    });
  });
