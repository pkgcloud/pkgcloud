...
exports.getFlavors = function getFlavors(callback) {
  ...
  oneandone.listHardwareFlavours(function (error, response, body) {
    ...
    oneandone.listBaremetalModels(function (errpr,repsonse,body){
        if (error) {
            callback(error);
            return;
        }
        ...
        var baremetalFlavors = JSON.parse(body);
        for( var i=0;i<baremetalFlavors.length;i++){
            flavors.push(baremetalFlavors[i])
        }
