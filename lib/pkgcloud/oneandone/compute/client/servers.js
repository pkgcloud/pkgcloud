...
// ####    @serverType   {String|Flavor} **Optional** defaults to 'cloud' and also supports 'baremetal'
...
exports.checkServerReady = function checkServerReady(serverId, callback) {
  var checkServer = {};
  oneandone.getServer(serverId, function (error, response, body) {
    ...
    if ((checkServer.status.state != oneandone.ServerState.POWERED_OFF
      ...
      setTimeout(function () {
        checkServerReady(checkServer.id, callback);
      }, 20000);
    } else {
      callback();
    }
