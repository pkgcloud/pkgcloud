...
exports.upload = function (options, callback) {
  ...
  if (options.headers['content-length'] !== undefined) {
    ...
    writeableStream = this.multipartUpload(options, success);
  }

  if (readableStream) readableStream.pipe(writeableStream);
...
exports.multipartUpload = function (options, callback) {
  var self = this,
    ...
    numBlocks = 0,
    chunksFinished = [],
    chunkedStream = new storage.ChunkedStream(chunk),
    ended = false;
  ...
  chunkedStream.on('data', function (data) {
    ...
    var next = function (err, body, res) {
      ...
      if (!ended) {
        ...
        self.sendBlockList(options, numBlocks, function(err, body, res) {
    ...
    var writeableStream = self.upload(options, next);
    writeableStream.write(data);
    writeableStream.end();
  });

  chunkedStream.on('end', function (data) {
    ended = true;
  });
