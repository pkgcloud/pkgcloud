...
exports.upload = function (options, callback) {
  ...
  try {
    blobClient.createBlockBlobFromStream(containerName, options.remote, fs.createReadStream(options.local), null, function(error){
      if(!error){
        callback(null,true);
      } else {
        callback(error);
...
exports.multipartUpload = function (options, callback) {
  ...
  callback(new Error('multipartUpload not implemented'));
  return;
...
exports.download = function (options, callback) {
  callback(new Error('download not implemented'));
  return;
