...
exports.getServers = function getServers(callback) {
  ...
  var getDeployment = function(service, next) {
    var ms = self._getManagementService();
    ms.getDeployment(service, service, function(err, res) {
      if(err) {
        next(err)
      } else {
        next(null,new compute.Server(self, res.body));
      }
    });
  };
  ...
  sm.listHostedServices(function(err, res) {
    if(err) {
      ...
      if (rspdata instanceof Array) {
        ...
        for (var i = 0; i < len; i++) {
          servers.push(rspdata[i].ServiceName);
        }
      } else if (rspdata) {
          servers.push(rspdata.ServiceName);
      }

      async.map(servers,getDeployment, function(err, res) {
        callback(err, res);
      });
    }
  });
