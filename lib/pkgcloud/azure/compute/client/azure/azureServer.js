...
var compute  = pkgcloud.providers.azure.compute;
...
//TODO: need to get this from the azure SDK
var hostOptions = {
...
AzureServer.prototype.createServer = function(options, next) {
  ...
  var self = this;
  var server = new AzureServerInfo();
...
AzureServer.prototype.getDeployment = function (serviceName, callback) {
  var sm = azure.createServiceManagementService(this.subscriptionId, this.auth, hostOptions),
    self = this,
    servers = [];
...
AzureServer.prototype.getServers = function (callback) {
  var self = this,
    services = [],
    servers = [],
    sm = azure.createServiceManagementService(this.subscriptionId, this.auth, hostOptions);
...
AzureServer.prototype.destroyServer = function(server, next) {
  ...
  async.waterfall([
    function(callback){
      ...
      self.findServer(server, callback);
    },
    function(server, callback){
      if(server === null) {
        // server not found on azure
        callback(null); // TODO: should we return an error if server not found?
      }
      // get the HostedService we need to create a VM
      self.getHostedService(callback);
    },
...
AzureServer.prototype.requestStatus = function(requestId, callback) {
  ...
  var sm = azure.createServiceManagementService(this.subscriptionId, auth, hostOptions),
    self = this;
...
var testServer = new AzureServerInfo('create-test-ids2','create-test-ids2');

var server = new AzureServer(config);
server.deleteServer(testServer, function(err, results) {
