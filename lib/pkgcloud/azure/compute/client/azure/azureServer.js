...
AzureServer.prototype.deleteOSDisk = function (name, callback) {

  var sm = azure.createServiceManagementService(this.subscriptionId, this.auth),
    self = this;

  console.log("deleting OS Disk:" + name);
  sm.deleteDisk(name, function(err, res) {
    if(err) {
      callback(err);
    } else {
      var requestId = res.headers['x-ms-request-id'];
...
AzureServer.prototype.deleteBlob = function (name, callback) {

  var sm = azure.createServiceManagementService(this.subscriptionId, this.auth),
    self = this;

  console.log("deleting OS storage blob:" + name);
  callback(null);

};

AzureServer.prototype.deleteVMStorage = function(serverInfo, next) {
  ...
  if(server && server.azure && server.azure.RoleList) {
    if(server.azure.RoleList[0] && server.azure.RoleList.OSVirtualHardDisk) {
      disk = server.azure.RoleList[0].OSVirtualHardDisk.DiskName;
      blob = server.azure.RoleList[0].OSVirtualHardDisk.MediaLink;
    }
  }
  ...
  async.waterfall([
    function(callback) {
      ...
      self.deleteOSDisk(disk, callback);
    },
    function(callback) {
      if(blob === null) {
        callback(null);
      } else {
        self.deleteBlob(blob, callback);
      }
    }],
    function (err, result) {
      next(err, result);
  ...
  async.waterfall([
    ...
    function(serverResult, callback) {
      if(serverResult === null) {
        return errs.handle(
          errs.create({ message: 'Cannot destroy Server' +  serverInfo.name + '. Server not found'}),
          callback
        );
      } else {
        server = serverResult;
    ...
    function(callback){
      ...
      self.deleteVMStorage(server, callback);
    },
    function(callback) {
