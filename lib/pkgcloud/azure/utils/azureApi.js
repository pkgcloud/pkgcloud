...
getHostedServices = exports.getHostedServices = function (client, callback) {
  ...
  client.get(path, function (err, body) {
    ...
    if (body && body.HostedServices && body.HostedServices.HostedService) {
      // need to check if azure returned an array or single object
      if (Array.isArray(body.HostedServices.HostedService)) {
        body.HostedServices.HostedService.forEach(function (service) {
          services.push(service);
        });
      } else {
        services.push(body.HostedServices.HostedService);
      }
    }
...
getServersFromService = function (client, serviceName, callback) {
  ...
  getHostedServiceProperties(client, serviceName, function (err, result) {
    ...
    if (result && result.HostedService && result.HostedService.Deployments && result.HostedService.Deployments.Deployment) {
      if (isVM(result.HostedService.Deployments.Deployment)) {
        servers.push(result.HostedService.Deployments.Deployment);
      }
    }
...
createLinuxVM = function (client, options, vmOptions, callback) {
  ...
  var configParams = {
    ...
    OS_SOURCE_IMAGE_NAME: vmOptions.image.OSImage.Name,
