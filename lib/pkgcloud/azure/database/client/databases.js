...
exports.create = function (options, callback) {
  ...
  async.waterfall([
    ...
    function (err, result) {
      return !err
        ? callback(null, self.formatResponse(result.entry))
        : callback(err);
    }
  );
};
...
exports.list = function (callback) {
  ...
  this._xmlRequest({
    ...
    if (body && body.feed && body.feed.entry) {
      if (Array.isArray(body.feed.entry)) {
        body.feed.entry.forEach(function (table) {
          tables.push(self.formatResponse(table));
        });
      } else {
        tables.push(self.formatResponse(body.feed.entry));
      }
    }
