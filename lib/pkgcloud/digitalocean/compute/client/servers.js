...
exports.getVersion = function getVersion(callback) {
  return errs.handle(
    errs.create({ message: "DigitalOcean's API does not support versioning" }), callback);
};
...
exports.getLimits = function getLimits(callback) {
  return errs.handle(
    errs.create({ message: "DigitalOcean's API is not rate limited" }), callback);
};
...
exports.createServer = function createServer(options, callback) {
  ...
  // FIXME(mmalecki) Don't hardcode region
  createOptions.qs.region_id = 1;
  ...
  // XXX(mmalecki) Integrate with existing keys API
  if (options.keyIds) {
    createOptions.qs.ssh_key_ids = options.keyIds.join(',');
  }
...
exports.destroyServer = function destroyServer(server, callback) {
  ...
  return this.request(stopOptions, function (err, body, res) {
    ...
    if (res.statusCode === 202) {
      ...
      function check() {
        ...
        self.request(checkOptions, function (err, body, res) {
          if (err) { return callback && callback(err) }
    ...
    function finish() {
      ...
      self.request(destroyOptions,  function (err, body, res) {
        return err
