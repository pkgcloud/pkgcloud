...
exports.destroyServer = function destroyServer(server, options, callback) {
  ...
  //
  // `destroy` should technically be an `_asyncRequest`, *however* `_asyncRequest`
  // is only actually useful when running multiple commands on a droplet in a
  // series. And, since we're killing the droplet...
  //
  this.request({
...
// Shuts down a server
//
exports.shutdownServer = function shutdownServer(server, callback) {
  var serverId = server instanceof base.Server ? server.id : server;
  return this._asyncRequest({
    path: '/droplets/' + serverId + '/shutdown'
  }, function (err, body, res) {
    return err
      ? callback(err)
      : callback(null, { ok: serverId }, res);
